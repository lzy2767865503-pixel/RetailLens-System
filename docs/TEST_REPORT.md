...
| Unit/API/security tests | Same | **Pass in two consecutive final-tree rounds: 11 files, 83 tests each** | The suite covers deterministic scoring, API isolation, desktop Host/Origin/Fetch Metadata rejection, command-line Host rejection, stable-origin navigation, PID-bound health, storage clearing, structured package-author/PE metadata source enforcement, strict packaged-DOM/candidate evidence, the portable PowerShell/TypeScript readiness-schema binding, real-PE/ZIP policy, four-domain eSigner CKA/no-PFX workflow policy, live-model binding gates, WACK approved executable tuple/private-record limitations, Store/release workflow hardening, and third-party signer negative cases |
...
| Windows package visual assets | Same | **Pass at source; native capture pending** | 1024-square icon and 310 x 150 wide tile rendered and visually inspected with no clipping. The Store workflow now requires four exact 1366 x 768 PNGs captured by the installed Round-2 QA candidate from built-in demo data, with candidate/nonce/hash/privacy/metadata gates; no native PNG is claimed until that Windows run succeeds |
...
| GitHub executable signing guard | Static/local validation; native signature run pending | **Implemented, publication blocked pending controlled boundary/certificate** | Protected manual workflow uses four distinct non-administrator same-host accounts: build, sign-only, no-signing signed verifier, and read-only publisher. A separately pinned SYSTEM broker quarantines each globally serialized fixed-NTFS handoff, removes the previous SID, requires zero source open handles, rehashes before granting the next SID, and the receiver independently checks exact SYSTEM-owned ACLs, reparse state, and the same whole-tree hash. The signer reports zero candidate executions, requires a SID+harness-hash+command signing-broker policy and schema-v3 cleanup evidence, then loses access before the verifier runs the signed archive twice. The publisher has only ReadAndExecute on verified bytes and is the sole job with `contents: write`. Release ownership exists only from exact HTTP 201 and every later operation uses frozen IDs. Native execution remains pending |
...
Windows runner. The final suite count is 83 tests across 11 files. Two consecutive
post-hardening local rounds on 2026-08-27 passed locked install, production/full
audits, all 83 tests, attribution, notices/SBOM, production build, Windows x64
...
| Exact production-identity AppX built on Windows | Pending; one unsigned original is preserved, a separate payload-equivalent QA copy is temporary-signed, and the workspace output is destroyed |
...
| Store candidate/evidence GitHub Actions artifact | Forbidden: the Store workflow has no artifact upload; the unsigned package plus lineage and verified screenshots remain only in the private exact-ACL handoff while QA, reports, records, certificate/private key, and run state are destroyed |
| Windows 10 x64 clean-machine test | Pending |
| Windows 11 x64 clean-machine test | Pending |
| Final Store screenshots from packaged app | Pending native Round-2 run; success requires four exact 1366 x 768 PNGs from the installed QA SHA using only built-in demo data. Concept images cannot satisfy this gate |
...
Do not mark this report **release-ready** until both Windows workflow rounds pass for the same protected-main commit and exact ZIP, every embedded release PE has one exact-author trusted/timestamped certificate thumbprint, both live OpenAI rounds pass for the exact review model/key/candidate, two complete WACK runs and lifecycle runs pass the unchanged signed Store QA package, the four exact candidate-derived 1366 x 768 screenshots pass their privacy/lineage gate, final hashes are recorded, private certification access and Product declarations are confirmed, and any Store certification result is linked. Local JSON is not an attestation, and uploading a package alone is not certification or publication.
