...
| GitHub executable signing guard | Static/local validation; native signature run pending | **Implemented, publication blocked pending controlled boundary/certificate** | Protected manual workflow uses three distinct same-host self-hosted accounts. Only the build account runs repository/dependency code; unsigned bytes move through a fixed-NTFS exact-ACL local handoff, never an Actions artifact. A fresh no-checkout signer invokes only a pinned out-of-repository machine-bound broker/harness, requires schema-v3 CKA install/uninstall/DeleteKey/provider/cert/key/master-key/AppData/process/registry cleanup evidence, and hands only signed bytes to a fresh no-checkout REST publisher. Release ownership exists only from an exact HTTP 201 response and all subsequent reads, assets, PATCH, verification, and rollback use frozen immutable IDs. Native execution is still pending |
